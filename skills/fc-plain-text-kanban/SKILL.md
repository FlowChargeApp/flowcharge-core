...
- **Edit a card's title/text/labels** → edit the record's `title`, body, or `tags`,
  regenerate.
- **Clear a tag WARN** → tags are validated against the canonical pool in `flowcharge/tags.md` by the index generator; edit the record's `tags` to a defined pool spelling, regenerate.
