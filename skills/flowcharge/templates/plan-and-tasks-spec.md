...
* Run `git diff --name-only <the plan's `base_commit`>..HEAD`, and read again only those files the plan names that also appear in that output. When that output is empty, read nothing again.
* When the plan carries no `base_commit`, read every file the plan names.
* Whenever you write a SEARCH/REPLACE block for a file, read that file first and copy the SEARCH text from it, whatever the two rules above decided.
