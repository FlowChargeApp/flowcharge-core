...
Canonical data model for the FlowCharge Core project-management suite. Every FlowCharge Core skill
...
Three properties of a `flowcharge/` tree are deliberately unconstrained. The generator
...
**Tag pool.** `flowcharge/tags.md` lists the canonical workstream tags, one per line
...
- `workstream`: `tags` — chosen from the canonical pool at `flowcharge/tags.md`:
  ...
  deliberately does not check this rule. It could: both triggers sit on disk, so the
...
is **not machine-checkable**: the intent to author into a workstream is nowhere on
