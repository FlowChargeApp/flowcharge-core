...
- `test_commands`, an optional task-list frontmatter key listing the test suites
  ...
  makes wrong and adds only tests the request asks for, then a test-gate task,
  which runs every recorded suite. A test-gate failure is filed as an issue and
...
- `fc-index.mjs --check` warns on a `done` task list with a non-empty
  `test_commands` that has no test-update task, no test-gate task, or a test-gate
  record other than `passed`.

### Changed

- A full test suite no longer counts as the user's step after execution: it runs
  only in the test-gate task.
